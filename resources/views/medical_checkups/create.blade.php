<x-layouts.app title="Nueva revisión médica">
    <div class="max-w-3xl mx-auto py-8 px-4">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Nueva Revisión Médica</h1>
        ...
        <form action="{{ route('medical_checkups.store') }}" method="POST" class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-lg p-6 shadow-sm">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" id="checkup_date" name="checkup_date" required
                           value="{{ old('checkup_date', now()->format('Y-m-d')) }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="observations" name="observations" rows="4"
                              class="block w-full rounded-md border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 shadow-sm focus:border-[#29b1dc] focus:ring focus:ring-[#29b1dc] focus:ring-opacity-50"
                              placeholder="Observaciones adicionales (opcional)">{{ old('observations') }}</textarea>
                    @error('observations')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
