                        ...
                        <tbody>
                            @foreach($payments as $p)
                                ...
                                <tr class="border-t hover:bg-gray-50 dark:hover:bg-zinc-800">
                                    <td class="px-3 py-3">{{ $p->payment_date ? \Carbon\Carbon::parse($p->payment_date)->format('d/m/Y') : '-' }}</td>
                                    <td class="px-3 py-3">{{ $periodLabel }}</td>
                                    <td class="px-3 py-3">{{ $p->student->name ?? '-' }}</td>
                                    <td class="px-3 py-3">{{ $p->paymentMethod->name ?? 'N/A' }}</td>
                                    <td class="px-3 py-3 text-right">${{ number_format($p->amount,2,',','.') }}</td>
                                    <td class="px-3 py-3">{{ $p->notes ?? '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
