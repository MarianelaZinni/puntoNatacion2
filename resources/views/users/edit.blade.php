...
<div>
    ...
    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">La edad se calcula automáticamente según la fecha de nacimiento</p>
            ...
            <div>
                <label for="active_from" class="block text-base font-medium text-gray-700 dark:text-gray-300">
                    Periodo de inicio
                    @if($activeFromRequired)<span class="text-red-500">*</span>@endif
                </label>
                <input
                    id="active_from"
                    name="active_from"
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Mes y año a partir del cual el alumno está activo y genera deuda.
                    @if(!$studentHasClasses) Puede dejarse vacío si el alumno no está inscripto en ninguna clase. @endif
                </p>
                @if($activeFromHasHistory)
                    <p class="mt-1 text-xs text-amber-700 dark:text-amber-300">
                        ⚠️ Este alumno ya tiene pagos registrados. Cambiar esta fecha puede afectar el cálculo de deuda histórica.
                    </p>
                @endif
                @error('active_from')
